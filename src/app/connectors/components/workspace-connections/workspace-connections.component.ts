...
export class WorkspaceConnectionsComponent implements OnInit, OnDestroy {
  connectorTypes: ConnectorType[] = [
    {
      ...
      name: 'PostgreSQL',
      description:
        'Relational database for transactional and analytical workloads.',
      icon: '',
  ...
  ngOnInit(): void {
    ...
    this.connectionSyncService.onSyncCompleted$
      ...
      .subscribe(() => {
        // Background sync finished (success or fail), refresh the list.
        this.loadConnections();
      });
  }
  ...
  openNewConnection(): void {
    const postgresConnector = this.connectorTypes.find(
      (c) => c.id === 'postgresql',
    );
  ...
  isSyncing(id: string): boolean {
    return this.connectionSyncService.isSyncing(id);
  }
  ...
  syncConnection(connection: DatabaseConnection, event: MouseEvent): void {
    ...
    this.activeDropdownId = null;
    this.connectionSyncService.startSync(
      connection.id,
  ...
  saveEditedConnection(updates: Partial<DatabaseConnection>): void {
    ...
    this.dbConnectionService
      .update(this.editingConnection.id, updates)
      .pipe(takeUntil(this.destroy$))
      .subscribe({
        next: (updatedConn) => {
          // Update connection in list
          const idx = this.connections.findIndex(
            (c) => c.id === updatedConn.id,
          );
          if (idx !== -1) {
